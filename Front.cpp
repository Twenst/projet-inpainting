...
void Front::defineFront(const ImgPixel& Img, std::list<Pixel> list_pixels)
{
    ...
    std::list<Pixel>::iterator iter = pixels.end();

    for (std::list<Pixel>::iterator i = list_pixels.begin(); i != list_pixels.end(); i++)
        if (i->getFilled())
            pixels.insert(iter,*i);
}

void updateFront(const ImgPixel& Img, std::list<Pixel> list_pixels)
{
